...
module i2c_slave (
	...
	input wire ack_nack,		// Determina se o escravo manda ACK (0) ou NACK (1). Quando 1 manda ACK, quando 0 manda NACK
	...
	reg rw;					  // Definir modo de leitura (1) ou escrita (0) do escravo
	reg error_address;    // Endereco invalido
    ...
    always @(*) begin
		...
		if (!start) begin
            ...
            case (state)
                IDLE: begin
                    if (~error_address && start && scl_last && !scl_sync) next_state = ADDR;
                end

                ADDR: begin
                    if (scl_last && !scl_sync) begin
                        if (bit_count == 0) begin
                            if (shift_reg[7:1] == ADDRESS) begin next_state = ACK;
                ...
                ACK: begin
	                if (scl_last && !scl_sync) begin
						if (ack_nack) begin next_state = (rw == 0) ? READ : WRITE;
						end else begin next_state = IDLE;
						end
					end
                end
                ...
                READ_ACK: begin
                    if (scl_last && !scl_sync) begin
                        if (sda_sync == 0) begin // ACK recebido
                            next_state = IDLE;
                        end else begin
                             next_state = WRITE; // NACK ou erro
						end
                    end
                end
            ...
            case (state)
                ...
                ADDR: begin
                    ...
                    if (scl_last && !scl_sync) bit_count <= bit_count - 1'd1;
						if (bit_count == 0) begin 
							rw <= shift_reg[0];
							if (shift_reg[7:1] == ADDRESS) begin error_address <= 0;
							end else begin error_address <= 1;
							end
						end
                end
