...
SIMULATION_FILE="$(mktemp "${TMPDIR:-/tmp}/mx_encoder.XXXXXX.vvp")"
...
python3 "$SCRIPT_DIR/encoder.py" "$INPUT_FILE" "$PYTHON_OUTPUT"

iverilog -g2012 -Wall \
    -s mx_encoder_tb \
    -o "$SIMULATION_FILE" \
    "$PROJECT_DIR/src/mx_encoder.v" \
    "$SCRIPT_DIR/mx_encoder_tb.v"
...
if [[ "$INPUT_FILE" -ef "$DEFAULT_INPUT" ]]; then
    if ! diff -u "$EXPECTED_OUTPUT" "$PYTHON_OUTPUT"; then
        echo "FAIL: a saida Python diverge do golden." >&2
        exit 1
    fi
    if ! diff -u "$EXPECTED_OUTPUT" "$VERILOG_OUTPUT"; then
        echo "FAIL: a saida Verilog diverge do golden." >&2
        exit 1
    fi
    echo "PASS: Python, Verilog e golden produziram resultados identicos."
else
